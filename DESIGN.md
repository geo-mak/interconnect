...
There are arguments floating around for a while for very "high-level" programming models as an answer to safety and complexity. The problem is that the higher a language goes, the more "specialized" it becomes. There is **no way** to do very "high-level" programming model without ending up having a DSL, and eventually needing many of those that get mixed in a single project, all while requiring an "escape hatch" to regain control and implement/reimplement things anyway. Higher "levelness" is better served with components provided by libraries for encapsulating the "control" details in a unified "hierarchal" programming model based on fine-grained control as its core **accessible** model, which can be encapsulated in "higher" level components (macros can be helpful..sometimes!).
